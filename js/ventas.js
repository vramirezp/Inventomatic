...
function ActualizarListaVentas(parametros)
{
	...
	$.post("modules/ventas.module.php", parametros, function(data){
		if(data != false)
		{
			$('#codigoF').val("");			
...
$(document).ready(function(){
	...
	$(document).on('click', '#cancelarC', function(e){
		...
		var answer = '¿Estás seguro de eliminar la venta completa?',
			cancel = 'Cancelar',
			accept = 'Aceptar';
	...
	$(document).on('click', '#guardar3', function(e){
		...
		var answer = '¿Estás seguro de guardar la venta completa?',
			cancel = 'Cancelar',
			accept = 'Aceptar';
